...
## 更新摘要
**所做更改**
- 新增完整的窗口管理系统，包括自定义最小化、最大化、关闭按钮
- 实现无边框窗口拖拽功能，提升用户体验
- 移除人脸识别图像显示区域，简化左侧面板布局
- 统一应用样式和调色板，通过QSS样式表实现一致的视觉风格
- 新增设置窗口功能，提供系统配置管理界面
- 优化摄像头显示区域，使用自定义FaceVideoWidget进行人脸框绘制
...
## 简介
本文档面向用户界面组件，系统化梳理考勤系统的主窗口布局、实时视频预览与人脸打卡业务流的UI设计理念与实现要点。经过重大重构后，系统从简单的800x600网格布局扩展为1299x892的复杂三段式左侧面板设计，新增窗口管理系统、自定义样式表、设置窗口等功能。文档覆盖以下方面：
- 视觉外观与布局：主窗口容器、三段式布局、顶部状态栏的组织方式
...
- 主窗口（QMainWindow）
  ...
  - 顶部状态栏包含时间显示、网络状态、设置按钮和自定义窗口控制按钮
...
- 视觉外观
  ...
  - 顶部状态栏采用水平布局，包含时间显示、网络状态、设置按钮和自定义窗口控制按钮
...
- QSS样式表统一管理
  - 主窗口背景色：#f5f5f5（浅灰色）
  - 顶部工具栏：深蓝色背景（#2c3e50）
  - 按钮样式：统一的蓝色主题，支持悬停和按下状态
  - 窗口控制按钮：透明背景，悬停时显示白色半透明背景
  - 输入框样式：浅色背景，只读状态使用灰色背景
- 固定调色板
  - 确保应用颜色不随系统主题变化
  - 统一的前景色、背景色、按钮色等
- 滚动条样式
...
## 结论
本UI设计经过重大重构后，从简单的网格布局演进为复杂的三段式左侧面板设计，显著提升了用户体验和信息展示能力。新的布局架构以"顶部状态栏 + 左右分栏 + 实时预览"为核心，通过模块化拆分实现了相机、视频帧处理与人脸识别的解耦。新增的窗口管理系统提供了现代化的窗口控制体验，统一的样式系统确保了视觉一致性，独立的设置窗口增强了系统的可配置性。当前实现聚焦功能完整性与可维护性，未来可在动画过渡、结果可视化框、主题切换等方面进一步增强用户体验。

## 附录

### 使用示例（步骤说明）
- 在主窗口中配置三段式布局（参考路径：[mainwindow.ui:101-231](file://mainwindow.ui#L101-L231)）
- 初始化窗口管理系统（自定义按钮、拖拽功能）
- 设置样式系统（QSS样式表、固定调色板）
