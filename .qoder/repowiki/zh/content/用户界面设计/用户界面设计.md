...
## 更新摘要
**所做更改**
- 更新了主窗口布局架构，从简单网格布局重构为复杂的三段式左侧面板设计
- 新增顶部状态栏区域，包含时间显示、网络状态和设备状态指示
- 左侧面板重构为员工信息展示区和人脸识别图像显示区
- 右侧面板新增摄像头实时画面显示区域
- 扩展了UI尺寸至970x815，提供更丰富的用户界面空间
- 增强了状态显示机制，包括打卡状态和识别结果反馈
...
## 简介
本文档面向用户界面组件，系统化梳理考勤系统的主窗口布局、实时视频预览与人脸打卡业务流的UI设计理念与实现要点。经过重大重构后，系统从简单的800x600网格布局扩展为970x815的复杂三段式左侧面板设计，新增时间展示、员工信息输入表单、打卡状态显示和摄像头实时显示等功能。文档覆盖以下方面：
- 视觉外观与布局：主窗口容器、三段式布局、状态栏的组织方式
...
## 核心组件
- 主窗口（QMainWindow）
  - 几何尺寸扩展至970x815，采用垂直布局组织顶部状态栏和中心内容区域
  - 顶部状态栏包含时间显示、网络状态、设备状态和设置按钮
...
### 主窗口（QMainWindow）
- 视觉外观
  - 几何尺寸扩展至970x815像素，提供更丰富的界面布局空间
  - 顶部状态栏采用水平布局，包含时间显示、网络状态、设备状态和设置按钮
  - 中心区域采用水平布局分为左侧面板（比例1）和右侧面板（比例2）
  - 左侧面板包含员工信息组框和人脸识别图像组框
  - 右侧面板包含摄像头实时画面显示区域
...
### 顶部状态栏组件
- 时间显示区域（timeLabel）
  - 采用12号加粗字体，实时显示当前日期和时间
  - 支持定时更新功能，提供准确的时间信息
- 网络状态指示（networkStatusLabel）
  - 绿色指示灯表示网络连接正常
  - 可扩展为动态状态更新，显示连接质量
- 设备状态指示（deviceStatusLabel）
  - 绿色指示灯表示设备运行正常
  - 可扩展为多种状态显示（连接中、异常等）
- 设置按钮（settingButton）
  - 集成配置功能入口
...
**章节来源**
- [mainwindow.ui:22-76](file://mainwindow.ui#L22-L76)
...
- 员工信息组框（infoGroupBox）
  ...
  - 支持动态更新，实时反映识别结果
- 人脸识别图像组框（faceImageGroupBox）
  - 显示识别到的人脸图像，最小尺寸200x200像素
  - 默认显示"暂无人脸识别"占位符
  - 支持图像更新和占位符切换

**章节来源**
- [mainwindow.ui:86-194](file://mainwindow.ui#L86-L194)

### 右侧面板组件
- 摄像头实时画面显示（cameraDisplay）
  - 最小尺寸640x480像素，支持自动缩放
  - 黑色背景配白色边框，提供良好的视觉对比
  - 默认显示"摄像头画面显示区域"占位符
  - 支持实时视频预览显示

**章节来源**
- [mainwindow.ui:211-233](file://mainwindow.ui#L211-L233)
...
## 结论
本UI设计经过重大重构后，从简单的网格布局演进为复杂的三段式左侧面板设计，显著提升了用户体验和信息展示能力。新的布局架构以"顶部状态栏 + 左右分栏 + 实时预览"为核心，通过模块化拆分实现了相机、视频帧处理与人脸识别的解耦。当前实现聚焦功能完整性与可维护性，未来可在样式主题、动画过渡、结果可视化框等方面进一步增强用户体验。

## 附录

### 使用示例（步骤说明）
- 在主窗口中配置三段式布局（参考路径：[mainwindow.ui:81-237](file://mainwindow.ui#L81-L237)）
