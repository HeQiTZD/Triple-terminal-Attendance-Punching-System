...
- Protocol：消息类型与数据结构定义
- Networkclient：业务编排与事件处理中枢
...
## 架构总览
整体采用“主控-子模块”架构，Networkclient作为中枢协调各子模块，通过信号槽解耦。Writer/Reader基于Socket双向数据通道，Heartbeat保障链路活性，MessageQueue提供可靠性保障。
...
## 结论
该网络客户端模块以清晰的职责划分与事件驱动架构实现了可靠的TCP通信：ConnectionManager提供稳健的连接与重连，HeartbeatManager保障链路活性，MessageQueue与Writer/Reader共同实现可靠的消息传输。配合Protocol的标准化消息格式与Networkclient的业务编排，系统具备良好的扩展性与可维护性。
...
- 协议解析
  ...
  - [Protocol::parseMessageType:50-61](file://NetworkClient/protocol.cpp#L50-L61)
- 错误处理
