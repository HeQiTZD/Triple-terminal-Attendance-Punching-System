...
bool LocalStorage::syncPersons(const QVector<ServerProtocol::PersonData> &persons)
{
    ...
    query.prepare("insert into Person (employee_id,name,face_feature,face_feature_size)"
                  "values(:employee,:name,:face_feature,:face_feature_size)");
...
bool LocalStorage::addAttendanceRecord(const QString &employeeId, const QString &status)
{
    ...
    QSqlQuery query(m_db);
    query.prepare("insert into AttendanceRecord (employee_id,status)"
...
QVector<ServerProtocol::AttendanceRecord> LocalStorage::getUnuploadedRecords()
{
    ...
    QSqlQuery query(m_db);
    query.prepare("select id,employee_id,check_time,status from AttendanceRecord"
                  "where uploaded = 0 order by check_time asc");
    ...
    while(query.next()){
        ServerProtocol::AttendanceRecord record;
        record.employeeId = query.value("employee_id").toString();
        record.checkTime  = query.value("check_time").toString();
...
bool LocalStorage::markAsUploaded(int recordId)
{
    ...
    QSqlQuery query(m_db);
    query.prepare("update AttendanceRecord set uploaded = 1, upload_time = datetime('now')"
                  "where id = :id ");
