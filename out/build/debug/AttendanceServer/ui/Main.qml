...
ApplicationWindow {
    ...
    function parseOptionalDateTime(text) {
    ...
    function describeRecord(record) {
        if (!record)
            return "空记录"
        const pn = record.personName !== undefined && record.personName !== null ? record.personName : ""
        return "#" + record.id + " | " + pn + " | employeeId=" + record.employeeId
    ...
    function describeFaceData(faceData) {
        if (!faceData)
            return "未找到人脸数据"
        const pn = faceData.personName !== undefined && faceData.personName !== null ? faceData.personName : ""
        return "#" + faceData.id + " | " + pn + " | employeeId=" + faceData.employeeId
             + " | size=" + faceData.featureSize
             + " | created=" + Qt.formatDateTime(faceData.createdAt, "yyyy-MM-dd HH:mm:ss")
             + " | updated=" + Qt.formatDateTime(faceData.updatedAt, "yyyy-MM-dd HH:mm:ss")
    }

    function refreshPersons() {
        personModel = dataService.getAllPerson()
        logResult("加载人员列表", true, "数量=" + personModel.length)
    }

    function refreshDevices() {
        deviceModel = dataService.getAllDevices()
        logResult("加载设备列表", true, "数量=" + deviceModel.length)
    }

    function refreshFaces() {
        faceModel = dataService.getAllFaceData()
    ...
    Connections {
        target: dataService
        function onOperationTip(message) { rememberInfo(message) }
    ...
    Connections {
        ...
        function onFaceDataAdded(id, employeeId) { appendLog("人脸数据已新增: id=" + id + " employeeId=" + employeeId) }
        function onFaceDataUpdated(id, employeeId) { appendLog("人脸数据已更新: id=" + id + " employeeId=" + employeeId) }
            ...
            ColumnLayout {
                ...
                GroupBox {
                    ...
                    GridLayout {
                        ...
                        Label { text: qsTr("数据库") }
                        Label { text: dataService && dataService.isConnected ? qsTr("已连接") : qsTr("未连接") }
                        Label { text: qsTr("TCP") }
                        ...
                        Label { text: qsTr("最后提示") }
                        Label {
                            Layout.columnSpan: 7
                            Layout.fillWidth: true
                            text: win.lastInfo.length ? win.lastInfo : qsTr("无")
                            color: palette.text
                            elide: Text.ElideRight
                        }
                    }
                                ...
                                ColumnLayout {
                                    ...
                                    RowLayout {
                                        ...
                                        Button {
                                            text: qsTr("新增")
                                            onClicked: {
                                                const ok = dataService.addPerson(personName.text, personEmployeeId.text, personDepartment.text, personPosition.text)
                                        ...
                                        Button {
                                            text: qsTr("修改")
                                            onClicked: {
                                                const ok = dataService.updatedPerson(personName.text, personEmployeeId.text, personDepartment.text, personPosition.text)
                                        ...
                                        Button {
                                            text: qsTr("删除")
                                            onClicked: {
                                                const ok = dataService.deletePerson(personEmployeeId.text)
                                                logResult("删除人员", ok, "工号=" + personEmployeeId.text)
                                                if (ok) refreshPersons()
                                            }
                                        }
                                        Button {
                                            text: qsTr("按 ID 查询")
                                            onClicked: {
                                                selectedPerson = dataService.getPersonById(parseIntOrZero(personId.text))
                                        ...
                                        Button {
                                            text: qsTr("按工号查询")
                                            onClicked: {
                                                selectedPerson = dataService.getPersonByEmployeeId(personEmployeeId.text)
                                ...
                                ColumnLayout {
                                    ...
                                    RowLayout {
                                        ...
                                        Button {
                                            text: qsTr("新增/更新设备")
                                            onClicked: {
                                                const ok = dataService.addOrUpdateDevice(deviceId.text, deviceName.text, deviceIp.text, deviceStatus.currentText)
                                        ...
                                        Button {
                                            text: qsTr("更新状态")
                                            onClicked: {
                                                const ok = dataService.updateDeviceStatus(deviceId.text, deviceStatus.currentText)
                                        ...
                                        Button {
                                            text: qsTr("按设备 ID 查询")
                                            onClicked: {
                                                selectedDevice = dataService.getDeviceById(deviceId.text)
                                        ...
                                        Button {
                                            text: qsTr("条件查询")
                                            onClicked: {
                                                const stFilter = deviceSelectStatusFilter.currentIndex === 0
                                                    ? ""
                                                    : deviceSelectStatusFilter.currentText
                                                deviceModel = dataService.selectDevice(
                                                    deviceId.text.trim(),
                                                    deviceName.text.trim(),
                                                    deviceIp.text.trim(),
                                                    stFilter)
                                                logResult("条件查询设备", true, "数量=" + deviceModel.length)
                                            }
                                        }
                                        Label { text: qsTr("状态筛选") }
                                        ComboBox {
                                            id: deviceSelectStatusFilter
                            ...
                            GroupBox {
                                ...
                                ColumnLayout {
                                    ...
                                    GridLayout {
                                        Layout.fillWidth: true
                                        columns: 8
                                        Label { text: qsTr("员工工号") }
                                        TextField { id: attendanceEmployeeId; text: "E001"; Layout.preferredWidth: 100 }
                                        ...
                                        Label { text: qsTr("姓名(模糊)") }
                                        TextField {
                                            id: attendancePersonName
                                    ...
                                    RowLayout {
                                        ...
                                        Button {
                                            ...
                                            onClicked: {
                                                const ok = dataService.addAttendanceRecord(
                                                    attendanceEmployeeId.text.trim(),
                                                    parseDateTime(attendanceCheckTime.text),
                                                    attendanceDeviceId.text,
                                                    attendanceStatus.currentText
                                                )
                                                logResult("新增考勤记录", ok, "employeeId=" + attendanceEmployeeId.text)
                                            }
                                        }
                                        Button {
                                            text: qsTr("按时间查询")
                                            onClicked: {
                                                attendanceModel = dataService.selectAttendanceRecord("", parseDateTime(attendanceStart.text), parseDateTime(attendanceEnd.text), "", "", "")
                                        ...
                                        Button {
                                            text: qsTr("按人员查询")
                                            onClicked: {
                                                attendanceModel = dataService.selectAttendanceRecord(attendanceEmployeeId.text.trim(), parseDateTime(attendanceStart.text), parseDateTime(attendanceEnd.text), "", "", "")
                                        ...
                                        Button {
                                            text: qsTr("组合条件查询")
                                            onClicked: {
                                                attendanceModel = dataService.selectAttendanceRecord(
                                                    attendanceEmployeeId.text.trim(),
                                                    parseDateTime(attendanceStart.text),
                                                    parseDateTime(attendanceEnd.text),
                                                    attendanceDeviceId.text.trim(),
                                                    attendanceStatus.currentText,
                                                    attendancePersonName.text.trim()
                                                )
                                                logResult("组合查询考勤", true, "数量=" + attendanceModel.length)
                                            }
                                        }
                                        Item { Layout.fillWidth: true }
                                    }
                                ...
                                ColumnLayout {
                                    ...
                                    RowLayout {
                                        ...
                                        Button {
                                            text: qsTr("查询")
                                            onClicked: {
                                                selectedFaceData = dataService.getFaceDataByEmployeeId(faceEmployeeId.text)
                                    ...
                                    GridLayout {
                                        Layout.fillWidth: true
                                        columns: 8
                                        Label { text: qsTr("创建时间起") }
                                        TextField {
                                            id: faceCreatedStart
                                            placeholderText: qsTr("可选 yyyy-MM-dd HH:mm:ss")
                                    ...
                                    RowLayout {
                                        Button {
                                            text: qsTr("条件查询人脸库")
                                            onClicked: {
                                                faceModel = dataService.selectFaceData(
                                                    faceEmployeeId.text.trim(),
                                                    parseOptionalDateTime(faceCreatedStart.text),
                                                    parseOptionalDateTime(faceCreatedEnd.text),
                                                    parseOptionalDateTime(faceUpdatedStart.text),
                                                    parseOptionalDateTime(faceUpdatedEnd.text))
                                                logResult("条件查询人脸库", true, "数量=" + faceModel.length)
                                            }
                                        }
                                        Item { Layout.fillWidth: true }
                                    }
        ...
        GroupBox {
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    Button {
                        text: qsTr("记录状态快照")
                        onClicked: appendLog("状态快照: db=" + dataService.isConnected + ", tcp=" + tcpServer.isRunning + ", clients=" + tcpServer.clientCount)
                    }
                    Item { Layout.fillWidth: true }
                }
